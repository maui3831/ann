...
def main():
    ...
    st.write("Train a perceptron on AND or NAND logic gates and test it with custom inputs.")
    ...
    if st.button("Train Perceptron"):
        with st.spinner("Training perceptron..."):
            ...
            with col2:
                st.metric("Final Loss", f"{np.mean((forward_prop(X, W_trained, b_trained)[0] - Y) ** 2):.4f}")
    ...
    if 'trained' in st.session_state and st.session_state.trained:
        st.write(f"Model trained on {st.session_state.gate_type} gate is ready for testing!")
        ...
        st.info("Please train the perceptron first using the 'Train Perceptron' button above.")
